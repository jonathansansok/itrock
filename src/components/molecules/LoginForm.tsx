...
export default function LoginForm() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!passOk)
      return toastError("Mínimo 8 caracteres, 1 mayúscula y 1 número.");

    const u = users.find((u) => u.email.toLowerCase() === email.toLowerCase());
    const hash = await sha256(password);
    if (!u || u.passwordHash !== hash)
      return toastError("Usuario no registrado o contraseña incorrecta.");
  ...
  return (
    <form onSubmit={submit} className="space-y-3" noValidate>
      ...
      {touched.email && !emailOk && (
        <TimedError
          text="Ingresá un email válido (ej. usuario@dominio.com)."
          ms={3000}
      ...
      {touched.password && !passOk && (
        <TimedError
          text="Mínimo 8 caracteres, con una mayúscula y un número."
          ms={3000}
      ...
      >
        <svg width="18" height="18" viewBox="0 0 48 48" aria-hidden="true">
          <path
            fill="#EA4335"
            d="M24 9.5c3.7 0 7.1 1.3 9.7 3.8l7.2-7.2C36.7 2.2 30.7 0 24 0 14.6 0 6.5 5.4 2.6 13.3l8.4 6.5C13 14 18 9.5 24 9.5z"
          />
          <path
            fill="#FBBC05"
            d="M46.5 24.5c0-1.6-.2-3.2-.6-4.7H24v9h12.8c-.6 3-2.4 5.5-5.1 7.3l7.9 6.1c4.7-4.4 6.9-10.8 6.9-17.7z"
          />
          <path
            fill="#34A853"
            d="M11 27.8c-1-3-1-6.2 0-9.2l-8.4-6.5C.9 15.6 0 19.7 0 24c0 4.3.9 8.4 2.6 12l8.4-6.5z"
          />
          <path
            fill="#4285F4"
            d="M24 48c6.5 0 12-2.1 16-5.7l-7.9-6.1c-2.2 1.5-5 2.3-8.1 2.3-6 0-11.1-4.5-12.9-10.5L2.6 36C6.5 42.6 14.6 48 24 48z"
          />
        </svg>
