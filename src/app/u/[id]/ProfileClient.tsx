...
export default function ProfileClient({ userId, initialPosts }: { userId: string; initialPosts: Post[] }) {
  ...
  const publications = useMemo(() => list.filter(p => p.userId === userId), [list, userId]);
  const saved = useMemo(() => list.filter(p => (p.savedBy ?? []).includes(userId)), [list, userId]);
  const comments = useMemo(() => list.flatMap(p => p.comments.filter(c => c.userId === userId).map(c => ({ post: p, comment: c }))), [list, userId]);

  return (
    <div className="mx-auto w-full max-w-[680px] px-3 sm:px-4 py-4">
      <div className="mb-4 flex gap-2">
        <button onClick={() => setTab("pubs")}  className={`rounded-full px-4 py-2 text-sm ${tab==="pubs"?"bg-neutral-800 text-white":"bg-neutral-900/50 text-neutral-300"}`}>Publicaciones</button>
        <button onClick={() => setTab("coms")}  className={`rounded-full px-4 py-2 text-sm ${tab==="coms"?"bg-neutral-800 text-white":"bg-neutral-900/50 text-neutral-300"}`}>Comentarios</button>
        <button onClick={() => setTab("saved")} className={`rounded-full px-4 py-2 text-sm ${tab==="saved"?"bg-neutral-800 text-white":"bg-neutral-900/50 text-neutral-300"}`}>Guardados</button>
      </div>
