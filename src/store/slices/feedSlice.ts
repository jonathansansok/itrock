...
const slice = createSlice({
  ...
  reducers: {
    ...
    addComment(state, action: PayloadAction<{ postId: string; comment: string; userId: string }>) {
      const p = state.posts.find(x => x.id === action.payload.postId);
      if (p) {
        p.comments.unshift({
          id: crypto.randomUUID(), postId: p.id, userId: action.payload.userId,
          text: action.payload.comment, createdAt: new Date().toISOString(),
        });
      }
    },
  },
